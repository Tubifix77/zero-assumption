...
- The contract's **"live lookup"** means: `WebSearch` and `WebFetch` (plus any available retrieval
  MCP tools). Prefer primary/authoritative sources. If none of these tools are available, follow the
  contract's refusal path — do not answer factual questions from memory.
